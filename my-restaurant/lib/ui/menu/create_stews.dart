...
import 'package:my_restaurant/ui/menu/create_desserts.dart';
...
class _CreateStewsState extends State<CreateStews> {
  ...
  void goNext() {
    if (selected.length <= 3) {
      widget.menu.stews = selected.values.toList();

      NavigationUtils.push(context, CreateDesserts(widget.menu));
    } else {
  ...
  Widget createList() {
    ...
    final viewMeals = meals.where((meal) => meal.type == 'Stew').toList();
