...
class _CreateDessertsState extends State<CreateDesserts> {
  ...
  void goNext() {
    if (selected.length <= 3) {
      widget.menu.desserts = selected.values.toList();

      NavigationUtils.push(context, CreateDrinks(widget.menu));
    } else {
  ...
  Widget createList() {
    ...
    final viewMeals = meals.where((meal) => meal.type == 'Dessert').toList();
