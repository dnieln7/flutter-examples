...
import 'package:my_restaurant/ui/menu/create_middles.dart';
...
class _CreateEntrancesState extends State<CreateEntrances> {
  ...
  void goNext() {
    if (selected.length <= 3) {
      final menu = Menu(entrances: selected.values.toList());

      NavigationUtils.push(context, CreateMiddles(menu));
    } else {
  ...
  Widget createList() {
    ...
    final viewMeals = meals.where((meal) => meal.type == 'Entrance').toList();
